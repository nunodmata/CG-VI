...
public:
    Distortion(Point Eye, Point At, const Vector Up, const int W, const int H, const float fovW, const float fovH, float k1, float k2) 
        : Eye(Eye), At(At), Up(Up), W(W), H(H), fovW(fovW), fovH(fovH), k1(k1), k2(k2) {
